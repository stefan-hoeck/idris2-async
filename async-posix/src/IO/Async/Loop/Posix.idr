...
record Poll where
  ...
  queues   : IOArray size (SnocList Task)
...
workST :
  ...
  -> Fin n
  -> (poll     : Poller)
  -> IOArray n (SnocList Task)
  -> IO Poll
workST me poll queues =
  runIO $ \t =>
    let alive # t := ref1 True t
        ...
        uprk  # t := ref1 [<] t
        spwn  # t := ref1 (nextFin me) t
        lock  # t := ioToF1 makeMutex t
        cond  # t := ioToF1 makeCondition t
     in W n me alive queues prkd uprk poll spwn tim sigh lock cond # t
...
parameters (s : Poll)
  ...
  unparkTok :
       SnocList Task
    -> SortedMap (Token World) Task
    -> List (Token World)
    -> IO1 (List Task)
  unparkTok st m []          t =
    let _ # t := write1 s.parked m t
     in (st <>> []) # t
  unparkTok st m (tok::toks) t =
    case lookup tok m of
      Nothing  => unparkTok st m toks t
      Just pkg => unparkTok (st:<pkg) (delete tok m) toks t

  getQueued : IO1 (List Task)
  getQueued t =
   let st # t := casupdate s.queues s.me (\x => ([<], x)) t
    in case casupdate1 s.unparked (\x => ([<], x)) t of
         [<] # t => (st <>> []) # t
         sk  # t =>
          let m # t := read1 s.parked t
           in unparkTok st m (sk <>> []) t
...
--        -- another queue.
--        S k => case ts of
--          tsk::tsks => let _ # t := tsk.act t in loop k tsks t
--          []        =>
--           let r # t := runDueTimers s.timer t
--            in case getQueued t of
--                 tsk::tsks # t => let _ # t := tsk.act t in loop k tsks t
--                 [] # t =>
--                  let _ # t := checkSignals s.signals t
