...
enq : Task -> ST -> (ST,Bool)
enq pkg (S asleep tsks) = (S asleep (tsks :< pkg), asleep)

deq : ST -> (ST,List Task)
deq (S a st) = (S a [<], st <>> [])

-- deqAndSleep : ST -> (ST,Maybe Task)
-- deqAndSleep st@(S asleep h t) =
--   case h of
--     x::y => (S False y t, Just x)
--     []   => case t <>> [] of
--       x::y => (S False y [<], Just x)
--       []   => (S True [] [<], Nothing)
-- 
-- stl : ST -> (ST,List Task)
-- -- stl st@(S asleep h t) =
-- --   case h of
-- --     x::y => (S asleep y t, Just x)
-- --     []   => case t <>> [] of
-- --       x::y => (S asleep y [<], Just x)
-- --       []   => (st, Nothing)
-- 
-- POLL_ITER : Nat
-- POLL_ITER = 16
...
--   steal x (S k) t =
--     case casupdate s.queues x stl t of
--       []    # t => steal (nextFin x) k t
--       h::tl # t =>
